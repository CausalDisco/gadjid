...
mod test {
    ...
    #[test]
    #[ignore]
    fn reachability_algos_agree_on_cpdags() {
        // anchors at parent directory of Cargo.toml
        let mut testgraphs = std::path::PathBuf::new();
        testgraphs.push("..");
        testgraphs.push("testgraphs");

        for pdag in 10001..=10010 {
            let full_path_to_pdag = testgraphs.join(format!("{}.CPDAG-100.mtx", pdag));
            let pdag = crate::test::load_pdag_from_mtx(full_path_to_pdag.to_str().unwrap());
            assert_reachability_algos_agree_on_graph(&pdag);
        }
    }
}
